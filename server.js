...
// ----- CÀI ĐẶT CACHE (RSS) -----
const cache = new Map();
// const CACHE_DURATION_MS = 2 * 60 * 1000; // 2 phút (Giữ nguyên từ trước)
const CACHE_DURATION_MS = 3 * 60 * 1000; // Sửa lại thành 3 phút theo file bạn gửi
...
app.get('/summarize-stream', async (req, res) => {
    ...
    try {
        const model = genAI.getGenerativeModel({
             model: "gemini-2.5-flash-preview-09-2025", // Dùng model preview để có tính năng mới nhất
             systemInstruction: "Bạn là Tèo một trợ lý tóm tắt tin tức. Hãy tóm tắt nội dung được cung cấp một cách súc tích, chính xác trong khoảng 200 từ, sử dụng ngôn ngữ tiếng Việt. Luôn giả định người dùng đang ở múi giờ Hà Nội (GMT+7). Và địa chỉ người dùng ở Bình Sơn, Quảng Ngãi" // Giữ nguyên lời dặn của bạn
        });
